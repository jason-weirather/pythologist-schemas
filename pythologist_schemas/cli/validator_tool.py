...
def main(args):
   ...
   if args.report_excel:
      ...
      report_json, report_success, report_errors  = excel_to_json(args.report_excel, \
                                                                    _fname,
                                                                    ['Population Percentages','Population Densities'], \
                                                                    ignore_extra_parameters=False)
      total_success = total_success and report_success
   ...
   ## Now we can run pythologist to get a light read on each sample.

   for sample_file in sample_files:
      _lightly_validate_sample(sample_file,analysis_json,project_json)
...
def _lightly_validate_sample(sample_file,analysis_json,project_json):

   sample_name = sample_file['sample_name']
   to_compare = {}
   for export in sample_file['exports']:
      to_compare[export['export_name']] = _lightly_validate_export(sample_name,export,analysis_json,project_json)
...
def _lightly_validate_export(sample_name,export,analysis_json,project_json):
   ...
   for image_frame in export['images']:
      to_compare['image_frame'] = _lightly_validate_image_frame(image_frame,export_name,analysis_json)

   return to_compare

def _lightly_validate_image_frame(image_frame,export_name,analysis_json):
   ...
   if analysis_json['parameters']['region_annotation_strategy'] == 'GIMP TSI':
   ...
   elif analysis_json['parameters']['region_annotation_strategy'] == 'GIMP Custom':
   ...
   to_compare = {
      'binary seg file difference':image_frame['image_data']['binary_segs_maps_tif']['sha256_hash'],
